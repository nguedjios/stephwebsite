...
{
	public class FORMSchoolYear : System.Windows.Forms.Form
	{
		//FORM VARIABLES
		MDI MDIFORM = new MDI();
		...
		public static FORMSchoolYear publicSYList;
		...
		private static FORMSchoolYear sForm = null;
		public static FORMSchoolYear  Instance()
		{
			if(sForm==null){sForm = new FORMSchoolYear();}

			return sForm;
		}

		public FORMSchoolYear(){InitializeComponent();}
		...
		private void FORMSchoolYear_Load(object sender, System.EventArgs e)
		{
			//Set Images
			setImages();
			...
			loadSchoolYear("SELECT tblSchoolYear.IndexSY, [YearStart] & ' - ' & [YearEnd] AS SY FROM tblSchoolYear ORDER BY [YearStart] & ' - ' & [YearEnd] ASC");
			setButtonTips();
			publicSYList=this;
		}

		public void loadSchoolYear(string sSQL)
		{
			long totalRow =0;
			daSYList.SelectCommand.CommandText = sSQL;
		...
		{
			loadSchoolYear("SELECT tblSchoolYear.IndexSY, [YearStart] & ' - ' & [YearEnd] AS SY FROM tblSchoolYear WHERE " + sField + " LIKE '%" + sSearch + "%' ORDER BY [YearStart] & ' - ' & [YearEnd] ASC");
		}
		...
		{
			FORMSchoolYear_Modify.ADD_STATE = true;
			FORMSchoolYear_Modify sForm = new FORMSchoolYear_Modify();
			sForm.ShowDialog();
		}
		...
		private void bttnDelete_Click(object sender, System.EventArgs e)
		{
			try
			{
				if(listView1.Items.Count > 0)
				{
					...
					{
						try{sFunctions.setOleDbCommand(cmdDelete,"DELETE FROM tblSchoolYear WHERE IndexSY = @getIndexSY","@getIndexSY", listView1.Items[listView1.FocusedItem.Index].SubItems[1].Text);}
		...
		private void bttnReload_Click(object sender, System.EventArgs e){loadSchoolYear("SELECT tblSchoolYear.IndexSY, [YearStart] & ' - ' & [YearEnd] AS SY FROM tblSchoolYear ORDER BY [YearStart] & ' - ' & [YearEnd] ASC");}
		...
		private void bttnEdit_Click(object sender, System.EventArgs e)
		{
			if(listView1.Items.Count > 0)
			{
				try
				{
					FORMSchoolYear_Modify.ADD_STATE = false;
					FORMSchoolYear_Modify.iSchoolYear=Convert.ToInt32(listView1.Items[listView1.FocusedItem.Index].SubItems[1].Text);
					FORMSchoolYear_Modify sForm = new FORMSchoolYear_Modify();
					sForm.ShowDialog();
